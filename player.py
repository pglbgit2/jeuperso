...
class Player(fighter.CHARACTER):
    def __init__(self, name:str, faction:str, gold:int = 0, HP:int =20, MaxHP:int =20, Stamina:int =5, magic:int =0, stamina_regeneration:int =5, race :str = "Human",  Equipment: List[Union[armors.ARMOR, weapons.WEAPON, weapons.RANGE_WEAPON]] = [], Inventory: List[items.ITEM] = [], skills : List[str] = list(defaultSkills.DEFAULT_SKILLS), dodge : float = 0.15, skillsLevel : Union[Dict[str,int], str] = {}, shotBonus : float = 0, raceResistance : Dict[str,float] = None, bodyBaseResistance : Union[int, Dict[str, int]]=0, counter : Union[Dict[str,int],str]= {}):
        super().__init__(name,faction,gold,HP,MaxHP,Stamina, magic,  stamina_regeneration, race, Equipment, Inventory, skills,dodge, skillsLevel,shotBonus, raceResistance, bodyBaseResistance=bodyBaseResistance)
        ...
        try:
            with open(path, "r") as file:
                ...
                if dictLine != None:
                    ...
                    try:
                        fighterDictStr["bodyBaseResistance"] = ast.literal_eval(fighterDictStr["bodyBaseResistance"])
                    except:
                        fighterDictStr["bodyBaseResistance"] = int(fighterDictStr["bodyBaseResistance"])
                    file.close()
